...
const onlyOddChars = (str) => {
  ...
  let newArray = strArray.filter(elem => strArray.indexOf(elem) % 2);
...
const allHappy = (arr) => {
  // Solution code here...
  let check = arr.reduce((acc, elem) => {
    acc = acc && elem.includes(':)');
    return acc;
  }, true);
  return check;
};

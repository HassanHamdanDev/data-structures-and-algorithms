...
function transformToLis(obj) {
  ...
  let newArray = [];
  for (let elem in obj) {
    newArray.push(`<li>${elem}: ${obj[elem]}</li>`);
  }
  return newArray;
}
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let five = input.map(elem => elem.filter(item => typeof (item) === 'number' && !(item % 5)));
  let powerByFive = five.map(elem => elem.map(item => Math.pow(2, item)));
  return powerByFive;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let genderArray = data.reduce((accumulator, elem) => {
    if (elem.gender === 'female' || elem.gender === 'male') {
      accumulator.push(elem.name);
    }
    return accumulator;
  }, []);
  let answer = genderArray.join(' and ');
  return answer;
};
